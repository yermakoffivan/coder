...
Writes to a user's MCP OAuth token rows (persisting a refreshed token, recording a refresh failure) instead use the per-owner subject `dbauthz.AsChatdTokenOwner(ctx, userID)`, whose only permission is user-level `ActionUpdatePersonal` scoped to that one owner ID; reads stay on the daemon-wide chatd subject. The invariant: daemon-wide chatd may read any user's MCP tokens, but may write a token only under the identity of the token's owner. Do not restore a site-wide personal-write grant or route these writes through `AsSystemRestricted`; either would reopen cross-account token writes from the shared daemon context.
